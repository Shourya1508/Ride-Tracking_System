...
<script src="cdn/js/jquery.min1.js"></script>
</head>
...
/* Set the content attribute for the closed eye icon */
...
<circle fill="url(#SVGID_5_)" cx="435.095" cy="184.986" r="63.504"/>
 ...
 <div class="toaster" id="emailChanged">
...
<div class="container">
  ...
  <form id="registrationForm" method="post" action="UserLogin" onsubmit="return validateForm();">
...
<span class="error">
 <%= emailV == null ? "" : emailV %>
    ...
    <br/>
    <input type="password" class="pwd" placeholder="Password"  name="pass" 
    id="password" value="<%= passvalue == null ? "" : passvalue %>"> 
<span class="eye-icon toggle-password" onclick="togglePasswordVisibility()">
  &#128065;
</span>
...
var emailCh = <%= request.getAttribute("emailChanged") %>;
console.log("Email changed:",emailCh);
if (emailCh) {
    // Show the toaster for 3 seconds
    var toaster = document.getElementById('emailChanged');
...
function togglePasswordVisibility(){
    ...
    if (passwordField.attr('type') === 'password') {
      ...
      eyeIcon.removeClass('closed').addClass('open');
    } else {
      passwordField.attr('type', 'password');
      eyeIcon.removeClass('open').addClass('closed');
    }
  }
    ...
    function validateForm() {
		...
		if (!emailValidation(email)) {
	        isValid = false;
	        showError('Enter Valid Email', 'email_ID');
	    }else if(email == ""){
	    	 isValid = false;
		        showError('Email field required', 'email_ID');
	    }
